...
<dialog class="video-modal">
  ...
  `,e.setAttribute("aria-label","Scroll to top"),e.title="Back to top",document.body.appendChild(e),window.addEventListener("scroll",function(){const t=window.pageYOffset||document.documentElement.scrollTop,o=window.innerHeight,a=document.documentElement.scrollHeight-o*1.8;t>a?e.classList.add("show"):e.classList.remove("show")}),e.addEventListener("click",function(){window.scrollTo({top:0,behavior:"smooth"})})});const X="bca6557ef64423ebe36f13a6f80e4fa5",ee="https://api.themoviedb.org",Te="https://image.tmdb.org/t/p/",E=document.querySelector(".my-movies-select"),M=document.querySelector(".load-btn"),T=document.querySelector(".movie-list"),Y=document.querySelector(".btn-link");let te=new Map,C=[],_=[];async function Ae(){try{(await A.get(`${ee}/3/genre/movie/list?api_key=${X}&language=en-EN`)).data.genres.forEach(t=>{te.set(t.id,t.name)})}catch(e){console.error("Error fetching movie genres:",e)}}function De(){if(!E)return;E.innerHTML="";const e=document.createElement("option");e.value="all",e.textContent="All Genres",E.appendChild(e),te.forEach((t,o)=>{const n=document.createElement("option");n.value=o,n.textContent=t,E.appendChild(n)})}async function J(){const e=localStorage.getItem("myLibrary");let t=[];if(e)try{t=JSON.parse(e)}catch(n){console.error("Error parsing localStorage 'myLibrary' data:",n),t=[]}T.innerHTML="";const o=t.map(n=>A.get(`${ee}/3/movie/${n}?api_key=${X}`).then(a=>a.data).catch(a=>(console.error(`Error fetching movie with ID ${n}:`,a),null)));C=(await Promise.all(o)).filter(n=>n!==null),P(C),Oe()}function P(e){if(_=e,_.length===0){T.innerHTML="<p>No movies found for the selected genre.</p>";return}const t=`
    ...
    `,o=_.map(n=>{const{poster_path:a,title:r,release_date:i,vote_average:s}=n,u=i?i.split("-")[0]:"N/A",k=a?`${Te}w500${a}`:"https://via.placeholder.com/500x750?text=No+Image",d=Math.round((s||0)/2),g=Array(5).fill(0).map((y,h)=>`
      ...
      <li class="movie-card">
        ...
        <div class="movie-info">
          ...
          <div class="movie-meta">${u}</div>
        </div>
        <div class="rating-stars">${g}</div>
      </li>
    `}).join("");T.innerHTML=t+o,Fe()}function Fe(){document.querySelectorAll(".movie-card").forEach((t,o)=>{t.addEventListener("click",()=>{const n=_[o];n&&D(n)})})}function Oe(){M&&Y&&(C.length===0?(M.classList.add("hiddenBtn","searchBtn"),M.textContent="Search movie",Y.href="/cinemania-project/./catalog.html",T.innerHTML="<p class='noMovieInfo'>OOPS... <br>We are very sorry!<br>You don’t have any movies at your library.</p>"):(M.classList.remove("hiddenBtn","searchBtn"),M.textContent="Load more"))}function Ne(){E&&E.addEventListener("change",e=>{const t=e.target.value;if(t==="all")P(C);else{const o=C.filter(n=>n.genres.some(a=>a.id===parseInt(t)));P(o)}})}document.addEventListener("DOMContentLoaded",()=>{document.body.id==="library-page"&&(document.addEventListener("libraryUpdated",()=>{J()}),Ae().then(()=>{De(),J(),Ne()}))});const ne="bca6557ef64423ebe36f13a6f80e4fa5",oe="https://api.themoviedb.org",L="https://image.tmdb.org/t/p/",I=document.querySelector(".upcoming-movie");let ae=new Map,j=null;async function Pe(){try{(await A.get(`${oe}/3/genre/movie/list?api_key=${ne}&language=en-EN`)).data.genres.forEach(t=>{ae.set(t.id,t.name)})}catch(e){console.error("Error fetching movie genres:",e)}}function je(e){return e.map(t=>ae.get(t)||"Unknown Genre").join(", ")}function Ge(){const e=new Date,t=e.getFullYear(),o=e.getMonth(),n=new Date(t,o,1),a=new Date(t,o+1,0),r=i=>{const s=i.getFullYear(),u=String(i.getMonth()+1).padStart(2,"0"),k=String(i.getDate()).padStart(2,"0");return`${s}-${u}-${k}`};return{startDate:r(n),endDate:r(a)}}function W(e,t){ie(t)?e.textContent="Remove from My Library":e.textContent="Add to My Library"}function re(e){if(!e){const y=document.createElement(`<div>
        <p>OOPS...
          We are very sorry!
          But we couldn’t find any movie.</p>
      </div>`);I.innerHTML=y;return}j=e;let t,o,n,a;const r=window.innerWidth;r>=1280&&e.backdrop_path?(t=`${L}w1280${e.backdrop_path}`,o=` ${L}w780${e.backdrop_path} 780w,
        ...
        (max-width: 767px) 280px`,a="movie-image-poster");const i=e.release_date?new Date(e.release_date).toLocaleDateString():"Unknown Release Date",s=e.vote_average?e.vote_average.toFixed(1):"-",u=e.vote_count?e.vote_count.toLocaleString():"-",k=je(e.genre_ids),d=e.title.toUpperCase();I.innerHTML=`
    <div class="upcoming-movie-container">
        ...
        <div class="upcoming-info">
          ...
          `,e.appendChild(l),l.addEventListener("click",()=>{D(a)})})}).catch(o=>{console.error("Error fetching data:",o),e.innerHTML="<p>Error loading movies.</p>",t.style.display="none"})})});const V="bca6557ef64423ebe36f13a6f80e4fa5";function Je(){const e=document.getElementById("search-input"),t=document.getElementById("clear-btn"),o=document.getElementById("movie-list"),n=document.getElementById("search-form"),a=document.getElementById("pagination"),r=document.getElementById("year-select");if(!e||!t||!o||!n||!a||!r){console.error("Gerekli HTML elementleri bulunamadı! Sayfanızın doğru yüklendiğinden emin olun.");return}let i=24,s=1,u=[];e.addEventListener("input",()=>{t.style.display=e.value?"inline":"none"}),t.addEventListener("click",()=>{e.value="",t.style.display="none",s=1,d()}),r.addEventListener("change",()=>{s=1,d()});function k(m,l,p){if(m){const c=new URL("https://api.themoviedb.org/3/search/movie");return c.searchParams.append("api_key",V),c.searchParams.append("query",m),c.searchParams.append("page",l),p&&c.searchParams.append("primary_release_year",p),c.toString()}else{const c=new URL("https://api.themoviedb.org/3/discover/movie");return c.searchParams.append("api_key",V),c.searchParams.append("sort_by","popularity.desc"),c.searchParams.append("page",l),p&&c.searchParams.append("primary_release_year",p),c.toString()}}async function d(){const m=e.value.trim(),l=r.value;try{const p=k(m,s,l),f=await(await fetch(p)).json();if(!f.results||f.results.length===0){o.innerHTML='<p style="grid-column: 1/-1; text-align: center;">Sonuç bulunamadı.</p>',i=1,h();return}i=f.total_pages>24?24:f.total_pages,g(f.results),h()}catch(p){console.error("Film verisi alınamadı:",p),o.innerHTML='<p style="grid-column: 1/-1; text-align: center;">Film verisi alınırken hata oluştu.</p>',i=1,h()}}n.addEventListener("submit",m=>{m.preventDefault(),s=1,d()});function g(m){u=m;const l=`
      <svg style="height:0; width:0; position:absolute" aria-hidden="true" focusable="false">
        <defs>
          <linearGradient id="starGradientFill" x1="0" y1="0" x2="1" y2="1">
            <stop offset="0%" stop-color="#F84119" />
    ...
    `;o.innerHTML=l+m.map(p=>{const{poster_path:c,title:f,release_date:v,vote_average:b}=p,ce=v?v.split("-")[0]:"N/A",de=c?`https://image.tmdb.org/t/p/w500${c}`:"https://via.placeholder.com/500x750?text=No+Image",me=Math.round((b||0)/2),pe=Array(5).fill(0).map((We,ue)=>`
              <svg class="star ${ue<me?"filled":""}" viewBox="0 0 24 24" >
                <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>
              </svg>`).join("");return`
