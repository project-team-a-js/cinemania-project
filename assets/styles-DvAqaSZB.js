import{a as M}from"./vendor-CJ4cOYKs.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))a(n);new MutationObserver(n=>{for(const o of n)if(o.type==="childList")for(const r of o.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&a(r)}).observe(document,{childList:!0,subtree:!0});function s(n){const o={};return n.integrity&&(o.integrity=n.integrity),n.referrerPolicy&&(o.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?o.credentials="include":n.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function a(n){if(n.ep)return;n.ep=!0;const o=s(n);fetch(n.href,o)}})();const C=document.querySelector(".menu-button"),m=document.querySelector(".mobile-menu");C.addEventListener("click",T);function T(){m.style.display!="none"?m.style.display="block":m.style.display="none",console.log(m.style.display)}const v=document.querySelector("body"),g=document.querySelector(".toggle");let O=localStorage.getItem("mode");O==="light"&&(v.classList.add("light"),g.classList.add("active"));g.addEventListener("click",()=>(v.classList.toggle("light"),v.classList.contains("light")?localStorage.setItem("mode","light"):localStorage.setItem("mode","dark")));g.addEventListener("click",()=>g.classList.toggle("active"));const x=document.querySelector(".modal"),N=document.querySelector(".footer-link"),U=document.querySelector(".modal-close-btn");function j(){x.style.display="flex"}function F(){x.style.display="none"}N.addEventListener("click",e=>{e.preventDefault(),j()});U.addEventListener("click",F);const S="bca6557ef64423ebe36f13a6f80e4fa5",E=document.getElementById("year-select"),G=new Date().getFullYear(),f=document.getElementById("search-input"),b=document.getElementById("clear-btn"),u=document.getElementById("movie-list"),H=document.getElementById("search-form");for(let e=G;e>=1900;e--){const t=document.createElement("option");t.value=e,t.textContent=e,E.appendChild(t)}f.addEventListener("input",()=>{b.style.display=f.value?"inline":"none"});b.addEventListener("click",()=>{f.value="",b.style.display="none",w()});document.addEventListener("DOMContentLoaded",()=>{w()});async function w(){try{const e=`https://api.themoviedb.org/3/trending/movie/week?api_key=${S}`,s=await(await fetch(e)).json();!s.results||s.results.length===0?u.innerHTML='<p style="grid-column: 1/-1; text-align: center;">Trend film bulunamadı.</p>':k(s.results)}catch(e){console.error("Trend filmler alınamadı:",e),u.innerHTML='<p style="grid-column: 1/-1; text-align: center;">Trend filmleri alırken hata oluştu.</p>'}}H.addEventListener("submit",async e=>{e.preventDefault();const t=f.value.trim(),s=E.value;if(!t&&!s){w();return}try{let a=`https://api.themoviedb.org/3/search/movie?api_key=${S}`;t&&(a+=`&query=${encodeURIComponent(t)}`),s&&(a+=`&year=${s}`);const o=await(await fetch(a)).json();!o.results||o.results.length===0?u.innerHTML='<p style="grid-column: 1/-1; text-align: center;">Hiçbir sonuç bulunamadı.</p>':k(o.results)}catch(a){console.error("Arama sırasında hata oluştu:",a),u.innerHTML='<p style="grid-column: 1/-1; text-align: center;">Arama sırasında hata oluştu.</p>'}});function k(e){const t=`
  ...
  `;u.innerHTML=t+e.map(s=>{const{poster_path:a,title:n,release_date:o,vote_average:r}=s,c=o?o.split("-")[0]:"N/A",l=a?`https://image.tmdb.org/t/p/w500${a}`:"https://via.placeholder.com/500x750?text=No+Image",d=Math.round((r||0)/2),y=Array(5).fill(0).map((p,i)=>`
    ...
    `}).join("")}const D="bca6557ef64423ebe36f13a6f80e4fa5",I="https://api.themoviedb.org",L=document.querySelector(".upcoming-movie");let A=new Map;async function P(){try{(await M.get(`${I}/3/genre/movie/list?api_key=${D}&language=en-EN`)).data.genres.forEach(t=>{A.set(t.id,t.name)})}catch(e){console.error("Error fetching movie genres:",e)}}function Y(e){return e.map(t=>A.get(t)||"Unknown Genre").join(", ")}function R(){const e=new Date,t=e.getFullYear(),s=e.getMonth(),a=new Date(t,s,1),n=new Date(t,s+1,0),o=r=>{const c=r.getFullYear(),l=String(r.getMonth()+1).padStart(2,"0"),d=String(r.getDate()).padStart(2,"0");return`${c}-${l}-${d}`};return{startDate:o(a),endDate:o(n)}}function _(e,t){q(t)?e.textContent="Remove from My Library":e.textContent="Add to My Library"}function J(e){let t,s,a,n;const o=window.innerWidth;o>=1280&&e.backdrop_path?(t=`https://image.tmdb.org/t/p/w1280${e.backdrop_path}`,s=` https://image.tmdb.org/t/p/w780${e.backdrop_path} 780w,
        ...
        (max-width: 767px) 280px`,n="movie-image-poster");const r=e.release_date?new Date(e.release_date).toLocaleDateString():"Unknown Release Date",c=e.vote_average?e.vote_average.toFixed(1):"",l=e.vote_count?e.vote_count.toLocaleString():"",d=Y(e.genre_ids),y=e.title.toUpperCase();L.innerHTML=`
    ...
    </div>`;const p=L.querySelector(".addLibrary");_(p,e.id),p.addEventListener("click",()=>{const i=parseInt(p.getAttribute("data-id"));let h=$();q(i)?(h=h.filter(B=>B!==i),localStorage.setItem("myLibrary",JSON.stringify(h))):z(i),_(p,i)})}async function K(){await P();const{startDate:e,endDate:t}=R();try{const a=(await M.get(`${I}/3/discover/movie`,{params:{api_key:D,"primary_release_date.gte":e,"primary_release_date.lte":t,sort_by:"popularity.desc"}})).data.results;if(a&&a.length>0){const n=Math.floor(Math.random()*a.length),o=a[n];J(o)}else L.innerHTML="<p>No upcoming movies found for this month.</p>"}catch(s){console.error("Error fetching upcoming movies:",s)}}function z(e){let t=$();t=t||[],t.includes(e)||(t.push(e),localStorage.setItem("myLibrary",JSON.stringify(t)))}function $(){const e=localStorage.getItem("myLibrary");return e?JSON.parse(e):[]}function q(e){return $().includes(e)}document.addEventListener("DOMContentLoaded",K);document.querySelectorAll('a[href^="/"]').forEach(e=>{const t=e.getAttribute("href");t.includes("/cinemania-project/")||e.setAttribute("href","/cinemania-project"+t)});document.querySelectorAll('object[data^="/"]').forEach(e=>{const t=e.getAttribute("data");t.includes("/cinemania-project/")||e.setAttribute("data","/cinemania-project"+t)});
//# sourceMappingURL=styles-DvAqaSZB.js.map
