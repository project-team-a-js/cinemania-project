import{a as T}from"./vendor-CJ4cOYKs.js";(function(){const t=document.createElement("link").relList;if(t&&t.supports&&t.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))o(n);new MutationObserver(n=>{for(const a of n)if(a.type==="childList")for(const i of a.addedNodes)i.tagName==="LINK"&&i.rel==="modulepreload"&&o(i)}).observe(document,{childList:!0,subtree:!0});function r(n){const a={};return n.integrity&&(a.integrity=n.integrity),n.referrerPolicy&&(a.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?a.credentials="include":n.crossOrigin==="anonymous"?a.credentials="omit":a.credentials="same-origin",a}function o(n){if(n.ep)return;n.ep=!0;const a=r(n);fetch(n.href,a)}})();const ie=document.querySelector(".menu-button"),D=document.querySelector(".mobile-menu");ie.addEventListener("click",le);function le(){D.classList.add("displayB")}const O=document.querySelector("body"),_=document.querySelector(".toggle");let ce=localStorage.getItem("mode");ce==="light"&&(O.classList.add("light"),_.classList.add("active"));_.addEventListener("click",()=>(O.classList.toggle("light"),O.classList.contains("light")?localStorage.setItem("mode","light"):localStorage.setItem("mode","dark")));_.addEventListener("click",()=>_.classList.toggle("active"));document.addEventListener("mouseup",function(e){D.contains(e.target)||D.classList.remove("displayB")});const L=document.querySelector(".team-modal"),U=document.querySelector(".footer-link"),j=document.querySelector(".team-modal .modal-close-btn");document.querySelector(".team-modal .team-container");document.querySelector(".team-modal .member-card");function de(){L&&(L.style.display="flex")}function G(){L&&(L.style.display="none")}U&&U.addEventListener("click",e=>{e.preventDefault(),de()});L&&L.addEventListener("click",e=>{e.target===L&&G()});document.addEventListener("keydown",e=>{e.key==="Escape"&&L&&L.style.display==="flex"&&G()});j&&j.addEventListener("click",G);let N,$=!1;function F(e){try{return(JSON.parse(localStorage.getItem("myLibrary"))||[]).includes(e)}catch(t){return console.error("Error checking library:",t),!1}}function q(e){const t=document.getElementById("movie-modal"),r=document.getElementById("modal-poster");e.poster_path?r.src=`https://image.tmdb.org/t/p/w500${e.poster_path}`:r.src="https://via.placeholder.com/300x450?text=No+Image",r.alt=e.title||e.name||"Film";const o=document.getElementById("modal-title");o.textContent=e.title||e.name||"No Title";const n=e.vote_average?e.vote_average.toFixed(1):"0.0",a=e.vote_count||0;document.getElementById("modal-rating").textContent=n,document.getElementById("modal-vote-count").textContent=a;const i=e.popularity?e.popularity.toFixed(1):"0.0";document.getElementById("modal-popularity").textContent=i;let l="No genre information";if(e.genres&&Array.isArray(e.genres))l=e.genres.map(y=>y.name).join(", ");else if(e.genre_ids&&Array.isArray(e.genre_ids)){const y={28:"Action",12:"Adventure",16:"Animation",35:"Comedy",80:"Crime",99:"Documentary",18:"Drama",10751:"Family",14:"Fantasy",36:"History",27:"Horror",10402:"Music",9648:"Mystery",10749:"Romance",878:"Science Fiction",10770:"TV Movie",53:"Thriller",10752:"War",37:"Western"};l=e.genre_ids.slice(0,3).map(c=>y[c]||"Unknown").join(", ")}document.getElementById("modal-genres").textContent=l,document.getElementById("modal-overview").textContent=e.overview||"No overview available. Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum. Sed ut perspiciatis unde omnis iste natus error sit voluptatem accusantium doloremque laudantium, totam rem aperiam, eaque ipsa quae ab illo inventore veritatis et quasi architecto beatae vitae dicta sunt explicabo. Nemo enim ipsam voluptatem quia voluptas sit aspernatur aut odit aut fugit, sed quia consequuntur magni dolores eos qui ratione voluptatem sequi nesciunt.";const h=document.querySelector(".add-to-library-btn");F(e.id)?h.textContent="Remove from my library":h.textContent="Add to my library";const v=h.cloneNode(!0);h.replaceWith(v),v.addEventListener("click",()=>{F(e.id)?(pe(e.id),v.textContent="Add to my library"):(ue(e.id),v.textContent="Remove from my library")}),t.classList.remove("hidden"),t.classList.add("show"),t.style.display="flex",document.body.style.overflow="hidden",$=!0}function ue(e){try{let t=JSON.parse(localStorage.getItem("myLibrary"))||[];t.includes(e)||(t.push(e),localStorage.setItem("myLibrary",JSON.stringify(t)));const r=document.querySelector(".add-to-library-btn");r.textContent="Added to Library!",r.style.background="var(--orange)",r.style.color="white",setTimeout(()=>{r.textContent="Remove from my library",r.style.background="transparent",r.style.color="var(--orange)"},100),document.dispatchEvent(new CustomEvent("libraryUpdated"))}catch(t){console.error("Error adding movie to library:",t),alert("Error adding movie to library. Please try again.")}}function pe(e){try{const r=(JSON.parse(localStorage.getItem("myLibrary"))||[]).filter(n=>n!==e);localStorage.setItem("myLibrary",JSON.stringify(r));const o=document.querySelector(".add-to-library-btn");o.textContent="Removed from Library!",o.style.background="var(--orange)",o.style.color="white",setTimeout(()=>{o.textContent="Add to my library",o.style.background="transparent",o.style.color="var(--orange)"},100),document.dispatchEvent(new CustomEvent("libraryUpdated"))}catch(t){console.error("Error removing movie from library:",t),alert("Error removing movie from library. Please try again.")}}function x(){const e=document.getElementById("movie-modal");e.classList.add("hidden"),e.classList.remove("show"),e.style.display="none",document.body.style.overflow="auto",$=!1}window.closeMovieModal=x;function me(){$||(clearTimeout(N),N=setTimeout(()=>{$||x()},500))}document.addEventListener("DOMContentLoaded",()=>{const e=document.getElementById("movie-modal"),t=document.querySelector(".close-modal");t&&(t.onclick=o=>{o.stopPropagation(),x()}),e.onclick=o=>{o.target===e&&x()};const r=e.querySelector(".modal-content");r&&(r.onclick=o=>{o.stopPropagation()}),document.addEventListener("keydown",o=>{o.key==="Escape"&&!e.classList.contains("hidden")&&x()}),e.addEventListener("mouseenter",()=>{clearTimeout(N)}),e.addEventListener("mouseleave",()=>{$||me()})});const w="bca6557ef64423ebe36f13a6f80e4fa5",M="https://api.themoviedb.org",b="https://image.tmdb.org/t/p/",z=async()=>{const e=`${M}/3/trending/all/day?api_key=${w}`;return(await fetch(e)).json()},ye=async e=>{const t=`${M}/3/movie/${e}/videos?api_key=${w}`;return(await fetch(t)).json()},ge=document.querySelector(".content"),R=document.querySelector(".trailer"),fe=({title:e,overview:t,name:r})=>{const o=document.createElement("div");return e!==void 0?o.innerHTML=`
    <h1 class="title">${e}</h1>
    <p class="description">
      ${t}
    </p>
...
<dialog class="video-modal">
  ...
  <iframe id="trailer-iframe" src="https://www.youtube.com/embed/${e}?enablejsapi=1&autoplay=1"
    ...
    `,t},ve=async e=>{const{results:t}=await z(),r=t.map(fe);ge.replaceChildren(r[e]),document.querySelector("header").style.background=`linear-gradient(to right,hsla(0, 0%, 7%, 1), hsla(0, 0%, 7%, 0)),url('https://image.tmdb.org/t/p/original/${t[e].backdrop_path}') no-repeat center center / cover`,be(t[e].id),Le(e)},be=async e=>{const{results:t}=await ye(e),r=document.querySelector(".button");if(t.length>0){const o=t[0].key;let n=document.querySelector(".video-modal"),a=document.getElementById("trailer-iframe");if(!n){const l=he({key:o});R.replaceChildren(l),n=document.querySelector(".video-modal"),a=document.getElementById("trailer-iframe")}const i=r.cloneNode(!0);r.replaceWith(i),i.addEventListener("click",function(){a.src=`https://www.youtube.com/embed/${o}?enablejsapi=1&autoplay=1`,n.showModal()}),n.addEventListener("click",l=>{l.target===n&&n.close()}),n.addEventListener("close",()=>{a&&(a.contentWindow.postMessage('{"event":"command","func":"pauseVideo","args":""}',"*"),a.src=`https://www.youtube.com/embed/${o}?enablejsapi=1`)})}else{const o=r.cloneNode(!0);r.replaceWith(o),o.addEventListener("click",function(){var a=document.createElement("dialog");a.classList.add("trailer-error"),R.replaceChildren(a),a.innerHTML=`
      <form method="dialog">
        <button class="video-modal-close">X</button>
      </form>
      <p>OOPS... We are very sorry! But we couldn’t find the trailer.</p>
      <img src="../img/IMG_9881 1.png" alt="">
      `,a.showModal(),a.addEventListener("click",i=>{i.target===a&&a.close()})})}},Le=async e=>{const{results:t}=await z(),r=t[e];document.querySelector(".button2").addEventListener("click",function(){q(r)})};ve(Math.floor(Math.random()*19));document.addEventListener("DOMContentLoaded",function(){const e=document.createElement("button");e.id="scroll-up-btn",e.className="scroll-up-btn",e.innerHTML=`
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M7 14L12 9L17 14" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
    </svg>
  `,e.setAttribute("aria-label","Scroll to top"),e.title="Back to top",document.body.appendChild(e),window.addEventListener("scroll",function(){const t=window.pageYOffset||document.documentElement.scrollTop,r=window.innerHeight,n=document.documentElement.scrollHeight-r*1.8;t>n?e.classList.add("show"):e.classList.remove("show")}),e.addEventListener("click",function(){window.scrollTo({top:0,behavior:"smooth"})})});const E=document.querySelector(".my-movies-select"),k=document.querySelector(".load-btn"),B=document.querySelector(".movie-list"),J=document.querySelector(".btn-link");let V=new Map,C=[],I=[];async function we(){try{(await T.get(`${M}/3/genre/movie/list?api_key=${w}&language=en-EN`)).data.genres.forEach(t=>{V.set(t.id,t.name)})}catch(e){console.error("Error fetching movie genres:",e)}}function Ee(){if(!E)return;E.innerHTML="";const e=document.createElement("option");e.value="all",e.textContent="All Genres",E.appendChild(e),V.forEach((t,r)=>{const o=document.createElement("option");o.value=r,o.textContent=t,E.appendChild(o)})}async function W(){const e=localStorage.getItem("myLibrary");let t=[];if(e)try{t=JSON.parse(e)}catch(o){console.error("Error parsing localStorage 'myLibrary' data:",o),t=[]}B.innerHTML="";const r=t.map(o=>T.get(`${M}/3/movie/${o}?api_key=${w}`).then(n=>n.data).catch(n=>(console.error(`Error fetching movie with ID ${o}:`,n),null)));C=(await Promise.all(r)).filter(o=>o!==null),A(C),ke()}function A(e){if(I=e,I.length===0){B.innerHTML="<p>No movies found for the selected genre.</p>";return}const t=`
    ...
    `}).join("");B.innerHTML=t+r,Me()}function Me(){document.querySelectorAll(".movie-card").forEach((t,r)=>{t.addEventListener("click",()=>{const o=I[r];o&&q(o)})})}function ke(){k&&J&&(C.length===0?(k.classList.add("hiddenBtn","searchBtn"),k.textContent="Search movie",J.href="./catalog.html",B.innerHTML="<p class='noMovieInfo'>OOPS... <br>We are very sorry!<br>You don’t have any movies at your library.</p>"):(k.classList.remove("hiddenBtn","searchBtn"),k.textContent="Load more"))}function Se(){E&&E.addEventListener("change",e=>{const t=e.target.value;if(t==="all")A(C);else{const r=C.filter(o=>o.genres.some(n=>n.id===parseInt(t)));A(r)}})}document.addEventListener("DOMContentLoaded",()=>{document.body.id==="library-page"&&(document.addEventListener("libraryUpdated",()=>{W()}),we().then(()=>{Ee(),W(),Se()}))});const S=document.querySelector(".upcoming-movie");let Z=new Map,P=null;async function xe(){try{(await T.get(`${M}/3/genre/movie/list?api_key=${w}&language=en-EN`)).data.genres.forEach(t=>{Z.set(t.id,t.name)})}catch(e){console.error("Error fetching movie genres:",e)}}function $e(e){return e.map(t=>Z.get(t)||"Unknown Genre").join(", ")}function Ce(){const e=new Date,t=e.getFullYear(),r=e.getMonth(),o=new Date(t,r,1),n=new Date(t,r+1,0),a=i=>{const l=i.getFullYear(),h=String(i.getMonth()+1).padStart(2,"0"),p=String(i.getDate()).padStart(2,"0");return`${l}-${h}-${p}`};return{startDate:a(o),endDate:a(n)}}function Y(e,t){Q(t)?e.textContent="Remove from My Library":e.textContent="Add to My Library"}function K(e){if(!S){console.log("Error: upcomingMovie not found.");return}if(!e){S.innerHTML=`<div>
        <p>OOPS...
          We are very sorry!
        ...
        (max-width: 767px) 280px`,n="movie-image-poster");const i=e.release_date?new Date(e.release_date).toLocaleDateString():"Unknown Release Date",l=e.vote_average?e.vote_average.toFixed(1):"-",h=e.vote_count?e.vote_count.toLocaleString():"-",p=$e(e.genre_ids),v=e.title.toUpperCase();S.innerHTML=`
    ...
    </div>`;const y=S.querySelector(".addLibrary");Y(y,e.id),y.addEventListener("click",()=>{const c=parseInt(y.getAttribute("data-id"));let u=H();Q(c)?(u=u.filter(d=>d!==c),localStorage.setItem("myLibrary",JSON.stringify(u))):_e(c),Y(y,c)})}async function Ie(){await xe();const{startDate:e,endDate:t}=Ce();try{const o=(await T.get(`${M}/3/discover/movie`,{params:{api_key:w,"primary_release_date.gte":e,"primary_release_date.lte":t,sort_by:"popularity.desc"}})).data.results;if(o&&o.length>0){const n=Math.floor(Math.random()*o.length),a=o[n];K(a)}else S.innerHTML="<p>No upcoming movies found for this month.</p>"}catch(r){console.error("Error fetching upcoming movies:",r)}}function _e(e){let t=H();t=t||[],t.includes(e)||(t.push(e),localStorage.setItem("myLibrary",JSON.stringify(t)))}function H(){const e=localStorage.getItem("myLibrary");return e?JSON.parse(e):[]}function Q(e){return H().includes(e)}function Be(e,t){let r;return function(...o){const n=this;clearTimeout(r),r=setTimeout(()=>e.apply(n,o),t)}}const Te=Be(()=>{P?K(P):upcomingMovieContainer.innerHTML=""},250);document.addEventListener("DOMContentLoaded",Ie);window.addEventListener("resize",Te);const X={method:"GET",headers:{accept:"application/json",Authorization:"Bearer eyJhbGciOiJIUzI1NiJ9.eyJhdWQiOiIzZDA5M2Y5MzcwZWVhYjYyZGRhZDEyMTViYTMzYzdkYyIsIm5iZiI6MTc0Mzc5MjUzNi41MTMsInN1YiI6IjY3ZjAyOTk4ZjVhODBhYTU0NTk5NTM5MCIsInNjb3BlcyI6WyJhcGlfcmVhZCJdLCJ2ZXJzaW9uIjoxfQ.kD8CV4_F8L2GVoP85qrIIvPuEascfTURrC1KNOeDj4Q"}};let ee={};function qe(){return fetch("https://api.themoviedb.org/3/genre/movie/list?language=en-US",X).then(e=>e.json()).then(e=>{e.genres.forEach(t=>{ee[t.id]=t.name})})}function De(e){return e.map(t=>ee[t]).filter(Boolean).slice(0,2)}document.addEventListener("DOMContentLoaded",()=>{localStorage.getItem("theme");const e=document.getElementById("weekly-cards"),t=document.getElementById("loader-weekly");qe().then(()=>{fetch("https://api.themoviedb.org/3/trending/all/day?language=en-US",X).then(r=>r.json()).then(r=>{const o=r.results.slice(0,3);e.innerHTML="",t.style.display="none",o.forEach(n=>{const a=n.poster_path?`https://image.tmdb.org/t/p/w500${n.poster_path}`:"https://via.placeholder.com/500x750?text=No+Image",i=n.title||n.name||"Untitled",l=De(n.genre_ids).join(", "),h=(n.release_date||n.first_air_date||"Unknown").split("-")[0],p=Math.round((n.vote_average||0)*10)/10,v=5,y=Math.floor(p/2),c=p%2>=1?1:0,u=v-y-c,d=[...Array(y).fill('<svg class="star-weekly full"><use xlink:href="#icon-star"></use></svg>'),...Array(c).fill('<svg class="star-weekly half"><use xlink:href="#icon-star-half"></use></svg>'),...Array(u).fill('<svg class="star-weekly empty"><use xlink:href="#icon-star-outline"></use></svg>')].join(""),s=document.createElement("div");s.className="card-weekly",s.style.backgroundImage=`url(${a})`,s.style.backgroundSize="cover",s.style.backgroundPosition="center",s.style.display="flex",s.style.objectFit="contain",s.style.alignItems="flex-end",s.style.borderRadius="10px",s.style.position="relative",s.style.overflow="hidden",s.style.boxShadow=" linear-gradient(180deg, rgba(0, 0, 0, 0) 63.48%, rgba(0, 0, 0, 0.9) 92.16%)",s.innerHTML=`
    ...
    `;r.innerHTML=u+c.map(s=>{const{poster_path:f,title:m,release_date:g,vote_average:te}=s,ne=g?g.split("-")[0]:"N/A",oe=f?`https://image.tmdb.org/t/p/w500${f}`:"https://via.placeholder.com/500x750?text=No+Image",re=Math.round((te||0)/2),ae=Array(5).fill(0).map((Ne,se)=>`
              <svg class="star ${se<re?"filled":""}" viewBox="0 0 24 24" >
                <polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/>
              </svg>`).join("");return`
